...
/*hanml 修改，
	修改tap事件，增加相对于屏幕位置的判断，和手指在屏幕上来回移动，最后退回，不能再触发 tap事件的判断。
*/
  $(document).ready(function(){
    ...
    $(document.body).bind('touchstart', function(e){
	  ...
	  touch.sx1 = e.touches[0].screenX
      touch.sy1 = e.touches[0].screenY
	  touch.triggerTap = true
	  ...
	  touch.sx2 = e.touches[0].screenX
      touch.sy2 = e.touches[0].screenY
	  if (touch.triggerTap){
		  if ( (Math.abs(touch.sx1 - touch.sx2) >15) || (Math.abs(touch.sy1 - touch.sy2) >15) )
		  {
				touch.triggerTap = false
		  }
	 }
      ...
      // normal tap
		...
		if (touch.triggerTap)	{
		  if ( (Date.now() - touch.last) > 100 ) touch.el.trigger('tap')	
...
//增加JQUER AJAX加载中功能的设置
$(function(){
	loading=$('<div  id="loading"><img src="templates_dsLoading.gif" /> <br />正在加载...</div>').css({
			display:'none',
			width:'100%',
			height:'auto',
			textAlign:'center',
	...
	loadError=$('<div  id="loadError">数据加载出错！</div>').css({
				display:'none',
				width:'100%',
				height:'auto',
				textAlign:'center',
